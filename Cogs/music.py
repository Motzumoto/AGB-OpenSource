...
class Music(commands.Cog, wavelink.WavelinkMixin, name="music"):
    ...
    async def connect(self, ctx, *, channel: typing.Union[discord.VoiceChannel, discord.StageChannel] = None):
...
#create a loop command with the class and wavelink
    @commands.command()
    async def loop(self, ctx, *, song: str):
        """Add a song to the queue to be played repeatedly."""
        #await ctx.send("This currently isnt working as we're experiencing a lot of issues right now. Please try again later.")
        #return
        try:
            await ctx.message.delete()
        ...
        if not self.is_privileged(ctx):
            return await ctx.send('Only admins and the DJ may use this command.', delete_after=15)
        if not song:
            return await ctx.send('You must specify a song to add to the queue.', delete_after=20)
        await player.queue.put(song)
        await ctx.send('Added {} to the queue.'.format(song), delete_after=20)
