...
class FormattedHelp(commands.HelpCommand):
    def __init__(self):
        super().__init__(command_attrs={
            'usage': '`tp!help (command/category)`'}, hidden=True)
    ...
    async def send_command_help(self, command):
        self.context
        e = discord.Embed(title=f"Help - {command.qualified_name} {random.choice(emojis.rainbow_emojis)}",
                          description=f"{command.help}\n[Add me]({config.Invite}) | [Join the server]({config.Server}) | [Vote]({config.Vote})", color=EMBED_COLOUR)
    ...
    async def send_group_help(self, group):
        ctx = self.context
        e = discord.Embed(title=f"Help - {group.qualified_name} {random.choice(emojis.rainbow_emojis)}",
    ...
    async def send_cog_help(self, cog):
        ctx = self.context
        e = discord.Embed(title=f"Help - {cog.qualified_name} {random.choice(emojis.rainbow_emojis)}", description=getattr(
            cog, '__doc__', discord.Embed.Empty), color=EMBED_COLOUR)
        e.set_footer(
            text=f'{self.context.bot.user.name} by Motzumoto, iPlay G, and Fearful')
        embeds = [e]
    ...
    async def send_bot_help(self, mapping):
        async with self.context.typing():
            ...
            nsfw_names = ''.join(
                f'`{name}`, ' for index, name in enumerate(nsfw_q))
            ...
            info_names = ''.join(
                f'`{name}`, ' for index, name in enumerate(info_q))
            ...
            fun_names = ''.join(f'`{name}`, ' for index,
                                name in enumerate(fun_q))
            ...
            music_names = ''.join(
                f'`{name}`, ' for index, name in enumerate(music_q))
            ...
            if self.context.channel.is_nsfw():
                ...
                embed = discord.Embed(
                    color=EMBED_COLOUR, description=f"{description}\n[Add me]({config.Invite}) | [Join the server]({config.Server}) | [Vote]({config.Vote})")
                embed.set_footer(
...
#         await self.context.send(embed=Embed(title="You can only view NSFW commands in an NSFW channel!", color=EMBED_COLOUR), delete_after=10)
